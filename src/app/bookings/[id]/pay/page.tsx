...
function CheckoutForm({ booking, onPaymentSuccess }: { booking: Booking, onPaymentSuccess: () => void }) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (paymentIntent && paymentIntent.status === 'succeeded') {
        // Payment succeeded, now update the booking status in Firestore
        try {
            ...
            onPaymentSuccess();
             ...
             setErrorMessage("Payment was successful, but we couldn't update your booking. Please contact support.");
        }
    }
