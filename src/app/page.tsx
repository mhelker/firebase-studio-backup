...
async function getPerformersData(): Promise<{ performers: Performer[], error?: string }> {
  if (!isFirebaseConfigured) {
    return { 
      performers: [], 
      error: "Firebase is not configured. Please add your API keys to src/lib/firebase.ts."
    };
  }
  try {
    ...
    return { 
      performers: [], 
      error: "Could not load performers from the database. Check Firestore rules or connectivity." 
    };
  }
}
...
async function getCustomersData(): Promise<{ customers: Customer[], error?: string }> {
  ...
  try {
    ...
    if (error.code === 'failed-precondition') {
      return { 
        customers: [],
        error: "A Firestore index is required for this query. Please create it in Firebase console."
      };
    }
    return { 
      customers: [], 
    ...
    <div className="space-y-16">
      ...
      >
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center p-4 text-primary-foreground">
          <h1 className="text-4xl md:text-5xl font-headline font-bold mb-6">Talent at Your Doorstep</h1>
          <p className="text-lg md:text-xl mb-8 max-w-2xl mx-auto">Discover and book amazing local performers for your events, parties, or just for fun!</p>
          <div className="space-x-4">
            <Button size="lg" asChild className="bg-accent hover:bg-accent/90 text-accent-foreground">
              <Link href="/performers">Find Performers</Link>
            </Button>
            <Button size="lg" className="bg-green-200 text-green-800 hover:bg-green-300" asChild>
