...
import { CalendarDays, DollarSign, MapPin, Users, Clock4, Award, Briefcase, PlayCircle, Loader2, Volume2 } from 'lucide-react';
...
export function PerformerDetailClient({ performer, reviews }: PerformerDetailClientProps) {
  ...
  const dataAiHintForImage =
    performer.dataAiHint ||
    (performer.talentTypes && performer.talentTypes.length > 0
      ? (performer.talentTypes || []).map((t) => t.toLowerCase()).join(' ')
      : 'performer profile');

  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <Card className="overflow-hidden shadow-xl">
        {/* --- Profile Images (supports multiple) --- */}
        {performer.imageUrls && performer.imageUrls.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 p-2">
            {performer.imageUrls.map((url, idx) => (
              <div key={idx} className="relative w-full aspect-square rounded-lg overflow-hidden">
                <Image
                  src={url}
                  alt={`${performer.name || 'Performer'} image ${idx + 1}`}
                  fill
            ...
            <div className="flex flex-wrap gap-2">
              {(performer.talentTypes || []).map((type) => (
                <Badge
                  key={type}
                  variant="secondary"
                  className="text-md px-3 py-1 bg-primary/10 text-primary"
                >
                  {type}
                </Badge>
              ))}
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-lg text-foreground/80">
              {performer.longDescription || performer.description}
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t">
            {/* ... same as before (talents, specialties, etc.) ... */}
            <div>
              <h3 className="font-semibold text-md mb-2 flex items-center">
                <DollarSign className="w-5 h-5 mr-2 text-primary" /> Price
              </h3>
              <p className="text-lg font-semibold">
                ${performer.pricePerHour || 0}
                <span className="text-sm font-normal text-muted-foreground">/hour</span>
              </p>
            </div>
          </div>

          {/* --- Videos (supports multiple) --- */}
          <div className="pt-6 border-t">
            <h3 className="text-xl font-headline font-semibold mb-4 flex items-center text-primary">
              <PlayCircle className="w-6 h-6 mr-2" /> Performances
            </h3>
            {performer.youtubeVideoIds && performer.youtubeVideoIds.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {performer.youtubeVideoIds.map((vid, idx) => (
                  <div
                    key={idx}
        ...
        <CardContent>
          {isClient ? (
            <BookingForm
              ...
              pricePerHour={performer.pricePerHour || 0}
            />
          ) : (
